...
def train_model(model, save_dir, task_id, train_x, train_y, valid_x, valid_y, batch_size=512, epochs=500):
    ...
    return final_train_acc, final_train_loss, final_valid_acc, final_valid_loss
...
def main():
    ...
    if os.path.isfile("output.csv"):
        ...
        output_csv['train_loss'] = 100.0
        output_csv['valid_acc'] = 0.0
        output_csv['valid_loss'] = 100.0
    ...
    for index, row in task.iterrows():
        ...
        train_acc, train_loss, valid_acc, valid_loss = train_model(model,save_dir,task_id,train_x,train_y,valid_x,valid_y,batch_size=512,epochs=500)


        with open(os.path.join(save_dir, 'readme.txt'), 'w') as f:
            f.write('task id = {}\ninput size = {}\nprediction k = {}\nfeature = {}\nlabel threshold = {}\nlstm units = {}\nlearning rate = {}\nepsilon = {}\nregularizer = {}\ndata set = {}'.format(task_id, input_size, pred_k, feature_num, label_threshold, lstm_units, lr, epsilon, regularizer, data_set))

        output_csv['train_acc'][index] = train_acc
        output_csv['train_loss'][index] = train_loss
        output_csv['valid_acc'][index] = valid_acc
        output_csv['valid_loss'][index] = valid_loss
