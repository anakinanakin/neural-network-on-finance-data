...
for index, row in task.iterrows():
	...
	task_dir = os.path.join(os.getcwd(), 'result2048/'+str(task_id))
	...
	plt.plot(valid_loss)
	plt.plot(test_loss)
	plt.title('Loss_{}'.format(task_id))
	plt.ylabel('Loss')
	plt.xlabel('Epoch')
	...
	plt.plot(valid_acc)
	plt.plot(test_acc)
	plt.title('Accuracy_{}'.format(task_id))
	plt.ylabel('Accuracy')
	plt.xlabel('Epoch')
	plt.legend(['Train', 'Valid', 'Test'])
	...
	plt.title('Precision_{}'.format(task_id))
	...
	plt.title('Recall_{}'.format(task_id))
	plt.ylabel('Recall')
	plt.xlabel('Epoch')
	plt.legend(['Train', 'Valid', 'Test'])
	...
	plt.plot(train_precision)
	plt.plot(train_f1)
	plt.title('Train evaluation_{}'.format(task_id))
	plt.ylabel('Score')
	plt.xlabel('Epoch')
	...
	plt.plot(valid_precision)
	plt.plot(valid_f1)
	plt.title('Valid evaluation_{}'.format(task_id))
	plt.ylabel('Score')
	plt.xlabel('Epoch')
	plt.legend(['Accuracy', 'Recall', 'Precision', 'F1'])
	...
	plt.plot(test_precision)
	plt.plot(test_f1)
	plt.title('Test evaluation_{}'.format(task_id))
	plt.ylabel('Score')
	plt.xlabel('Epoch')
	plt.legend(['Accuracy', 'Recall', 'Precision', 'F1'])
