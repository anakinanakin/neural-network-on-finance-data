...
for index, row in task.iterrows():
    ...
    train_acc, valid_acc, test_acc = train_model(model, save_dir, task_id, train_x, train_y, valid_x, valid_y, test_x, test_y, batch_size=32, epochs=500)


    with open(os.path.join(save_dir, 'readme.txt'), 'w') as f:
        f.write('task id = {}\ninput size = {}\nprediction k = {}\nfeature = {}\nlabel threshold = {}\nlstm units = {}\nlearning rate = {}\nregularizer = {}\ndata set = {}'.format(task_id, input_size, pred_k, feature_num, label_threshold, lstm_units, lr, regularizer, data_set))


    #print(train_acc)
    output_csv['train_acc'][index] = train_acc
    output_csv['valid_acc'][index] = valid_acc
    output_csv['test_acc'][index] = test_acc
